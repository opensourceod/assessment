...
export default function GapReport() {
  ...
  const tasa =
    reporte.total_evaluadores > 0
      ? Math.round(
          (reporte.evaluadores_completados /
            reporte.total_evaluadores) *
            100
        )
      : 0
...
>
        ...
        {reporte.evaluadores_completados === 0 && (
          <div className="card bg-cyan-50 border-cyan-200 mb-6">
            <p className="text-sm text-dark">
              No evaluator responses yet. The radar chart
              will appear once at least one evaluator
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {[
            {
              label: 'Evaluators invited',
              value: reporte.total_evaluadores,
            },
            {
              label: 'Responses received',
              value: reporte.evaluadores_completados,
            },
            {
            ...
            },
            {
              ...
              value: reporte.categorias.length,
            },
          ].map((s) => (
            <div
              key={s.label}
        ...
        <div id="table-pdf" className="card">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100">
                  <th className="text-left py-2 font-semibold text-dark">
                    Dimension
                  </th>
                  ...
                  <th className="text-center py-2 font-semibold text-orange-600">
                    Manager
                  </th>
                  ...
                  <th className="text-center py-2 font-semibold text-green-600">
                    Friend
                  </th>
                </tr>
              </thead>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="text-center py-3">
                        {cat.self_score?.toFixed(2) ??
                          '—'}
                      </td>

                      <td className="text-center py-3">
                        {cat.external_score?.toFixed(2) ??
                          '—'}
                      </td>

                      <td
                        className={`text-center py-3 ${gapColor}`}
                      >
                        {gap != null
                          ? parseFloat(gap) > 0
                            ? `+${gap}`
                      ...
                      <td className="text-center py-3 text-orange-600">
                        {cat.score_manager?.toFixed(2) ??
                          '—'}
                      </td>

                      <td className="text-center py-3 text-blue-600">
                        {cat.score_colleague?.toFixed(2) ??
                          '—'}
                      </td>

                      <td className="text-center py-3 text-green-600">
                        {cat.score_friend?.toFixed(2) ??
                          '—'}
                      </td>
                    </tr>
                  )
          ...
          <p className="text-xs text-muted mt-4">
            Gap = Self − External. Positive gap means
            you rate yourself higher than others do
